...
> Словарь - упорядоченная изменяемая последовательность именованных жлементов.
...
Словари можно получить и из других типов, но из-за структуры это сделать немного сложнее. Главное, соблюсти основное условие - объект который вы хотите преобразовать должен сам быть типом массива.
...
В кортеж можно преобразовать то же, что и в список из - за схожести со списком.
...
Так же у множеств полностью отстуствует индексация. То есть нельзя обратиться к какому - то конкретному значению.
...
> Нельзя создать пустое множество с помощью данной записи, даннй объект не будет словарем.
...
### Сокращенные математическе операторы
...
Оператор, который сравнивает оба значения и выводит `True`, если оба объекта идентичны друг другу, а `False`, если один из объектов отличается от второго.
...
Оператор, который возвращает True, если левый объект всегда больше правого. False, если наоборот.
...
Оператор, который возвращает True, если левый объект всегда меньше правого. False, если наоборот.
...
Оператор, который возвращает True, если левый объект меньше или равен прравому. False, если наоборот.

```python
min_eq1 = 200 <= 500  # True
min_eq2 = 125 <= 125  # True
min_eq3 = 200 <= 120  # Fasle
```
...
#### 3. Функция пассива ( `pass` )

Данная функция настьлько ленива, что ничего не делает.


### Операторы массивов

Массивы так же бывают со своими операторами.

#### 1. Длинная массива ( `len()` )

Данный массив возвращает длинну массива.
